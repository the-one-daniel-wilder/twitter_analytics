...
module TweetAverage
  ...
  def average_day_of_week(start_time = 0, simple = false)
    ...
    current = parse_time(start_time) - WEEK * @range
    total = 0
    ...
    average_tweets = (total / @range.to_f).round(2)
  ...
  def average_week(start_time = 0, simple = false)
    range_start = parse_time(start_time) - WEEK * @range
    total = 0
  ...
  def average_range(start_time, finish_time = Time.new, simple = false)
    ...
    average_tweets = (total / @range.to_f).round(3)
